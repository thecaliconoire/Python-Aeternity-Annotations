...
class AEName:
    """provides a mapping from human-readable names"""
    Status = NameStatus
    
    def __init__(self, domain, client=None):
        """pulls the client info in this example client = EPOCHCLIENT as default"""
        if client is None:
            client = EpochClient()
    ...
    @property
    def b58_name(self):
        """Parameter B58 Name: Generates a base 58 hash based on claimed AEName"""
        return self._encode_name(self.domain)
    def get_name_hash(self):
        """returns the B58 hash name"""
        return AEName.calculate_name_hash(self.domain)

    @classmethod
    def calculate_name_hash(cls, name):
        """How to calculates the name hash with NamehashAlgorithm (BLAKE2b) based on ENS 
        Name to name code is the ASCII Alphanumeric conversion, of a name to a numeric name
        to understand B58 further please find a seperate tutorial"""
    ...
    def validate_pointer(cls, pointer):
        """cls= Class: Holds class of itself so it is inherits defined attributes
            Pointer= a dictionary with all the values the entry points towards"""
        return (
    ...
    def validate_address(cls, address, raise_exception=True):
        ...
        if not address.startswith(('ak$', 'ok$')): 
            if raise_exception:
                raise ValueError(
                    'pointer addresses must start within ak$'
                )
            return False
        return True
    ...
    def validate_name(cls, domain, raise_exception=True):
        """Name Validation is completed in accordance too IDNA2008 
        to generate hash it will be stored in the ACSII form""" ##Still needs to be done.
    ...
    def update_status(self):
        """ name_ttl (Client_ttl) a suggestion as to how long any clients should cache this information
            If the name is already claimed a warning to the client will be sent
            and the name will not be updated. 
            If name is unclaimed the processes is as follows: Pre-claim->Claim->updated"""
        try:
            #example uses the openapi client inside the epoch client
    ...
    def check_claimed(self):
        """If claimed generate error if not update name"""
        self.update_status()
    ...
    def full_claim_blocking(self, keypair,
        ...
        :parameter keypair: the keypair of the account registering the name
    ...
    def _get_pointers_json(self, target):
        """retrieves pointers (the addresss associated with the name) then serilizes the pointer"""
