...
##CTX allows for dictionary items too be read or set by acesses notations.
##__MAIN__.py is used to define context variables, and to trigger the context variables.
...
def _keypair(password=None):
    ...
    try:
        ...
        exit(1)
##from click context retieve the utility function and return a keypair (public&private) as well as the key path
##Private key derives the public key.
##The public key is used for peers to identify your account
##Public key is used too generate the hash function.
...
##Recap of prefixes: TH=Transaction Hash, AK=Account address, bh=Block Hash.
...
def _pp(data, title=None, prefix=''):
    ...
    VERBOSE=optional argument that provides extra info on the processes and operations of your code it is a double check"""
    ctx = click.get_current_context()
...
def wallet_create(ctx, password, force):
    """Create context, wallet password, and keypair"""
    kp = KeyPair.generate()
    kf = ctx.obj.get(CTX_KEY_PATH)
...
def wallet_save(ctx, private_key):
    """Save Wallet context variables path and private key"""
    try:
...
def wallet_address(password):
    """simply sets the wallets password"""
    kp, kf = _keypair(password=password) ##Why is "kf" (Key path) an undefined variable?
    _pp([
        ('Wallet address', kp.get_address()),
    ])
...
def name_register(ctx, name_ttl, ttl):
    """Register Preclaim, claim name from Aename then retrieve account key name"""
    try:
...
def name_transfer():
    ...
    print("transfer name")
    pass
# MORE INFO: https://github.com/aeternity/protocol/blob/master/AENS.md
...
def oracle_query():
    print("query oracle")
    pass
#MORE INFO: https://github.com/aeternity/protocol/blob/master/oracles/oracles.md
